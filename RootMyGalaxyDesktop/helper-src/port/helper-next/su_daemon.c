...
static const char *kernelsu_loader_path(void) {
  const char *override = getenv("S25U_KSUD_PATH");
  if (override && override[0] == '/') {
    return override;
  }
  return KSU_LOADER_PATH;
}

static int validate_aarch64_loader(const char *path) {
  struct stat st;
  Elf64_Ehdr header;
  ...
  ssize_t length = read(fd, &header, sizeof(header));
  int stat_ok = fstat(fd, &st) == 0;
  close(fd);

  return stat_ok && S_ISREG(st.st_mode) && st.st_size >= (off_t)sizeof(header) &&
         (st.st_mode & 0111) != 0 && length == (ssize_t)sizeof(header) &&
...
static int run_kernelsu_late_load(struct su_request *request, int conn) {
  ...
  if (pid == 0) {
    ...
    const char *loader_path = kernelsu_loader_path();
    if (!validate_aarch64_loader(loader_path)) {
      dprintf(STDERR_FILENO, "late-load: invalid AArch64 ELF: %s\n",
              loader_path);
      _exit(15);
    }
    ...
    if (loader == 0) {
	  ...
	  execl(LOGCAT_PATH, "logcat", "late-load", "--kmi",
	        "android13-5.15", "--package-name", "me.weishu.kernelsu",
	        (char *)NULL);
...
static int daemon_main(void) {
  ...
  if (chown(BOOTSTRAP_SOCK_PATH, allowed_client_uid, allowed_client_uid) != 0 ||
      chmod(BOOTSTRAP_SOCK_PATH, 0600) != 0 || listen(fd, 16) != 0) {
    close(fd);
    unlink(BOOTSTRAP_SOCK_PATH);
    return 1;
  }
...
int main(int argc, char **argv) {
  ...
  if (argc >= 2 && strcmp(argv[1], "--probe") == 0) {
    return rmg_target_validate(stdout, 1) ? 0 : ENODEV;
  }
  if (argc >= 2 && strcmp(argv[1], "--version") == 0) {
    puts("rmg-open 0.1 profile=" TARGET_PROFILE_ID);
    return 0;
  }
